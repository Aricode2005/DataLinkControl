...
Delay probability simulates packets getting temporarily trapped in network congestion, causing their transit time to exceed the receiver's \texttt{MAX\_LIFETIME} or the sender's timeout threshold.

In Stop-and-Wait, a delayed ACK causes a premature timeout. The sender retransmits the frame, only for the original, delayed ACK to arrive moments later. This creates a cascade of "Ghost ACKs" that momentarily confuse the sender's sliding window, resulting in wasted transmissions.

Go-Back-N performs terribly under high delay for the same reason it fails under high errors. If a packet is delayed beyond the receiver's acceptance threshold, it is dropped. Consequently, the entire pipeline collapses, and the entire window must be retransmitted. 

Selective Repeat handles random delays exceptionally well. Because it relies on independent timers, if one specific packet is delayed, only that packet's timer expires. The sender retransmits it while the rest of the window continues operating smoothly. The out-of-order buffering at the receiver absorbs the delay without halting the entire data stream.
